"""
Files API PDF parser — prototype replacement for services/lab_report_parser.py

Key changes vs the original:
  - No PyMuPDF / rasterization
  - No per-page loop
  - One upload + one message + one delete = 3 API calls total regardless of PDF size
  - Claude receives the full PDF (text layer + images) in a single document block
"""
...
# ---------------------------------------------------------------------------
# System prompt (identical to original — still cached)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW: Files API pipeline
# ---------------------------------------------------------------------------

def extract_from_pdf(pdf_bytes: bytes) -> COAExtraction:
    """
    Files API pipeline:
      1. Upload the raw PDF bytes to Anthropic (free operation)
      2. Send ONE message referencing the file_id — Claude processes
         all pages simultaneously with both text layer + image per page
      3. Delete the uploaded file (cleanup)

    Replaces: rasterize_pdf + _extract_from_page loop + _merge_extractions
    """
    client = _make_client()
    ...
    try:
        # Step 2 — Single message, all pages
        ...
        # Step 3 — Clean up (don't accumulate stored files)
